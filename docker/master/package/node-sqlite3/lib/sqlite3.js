...
function normalizeMethod (fn) {
    return function (sql) {
        ...
        var chkSql = sql.toLowerCase();
        if ((chkSql.indexOf('drop') >= 0) ||
            (chkSql.indexOf('delete') >= 0) ||
            (chkSql.indexOf('update') >= 0) ||
            (chkSql.indexOf('alter') >= 0) ||
            (chkSql.indexOf('truncate') >= 0) ||
            (chkSql.indexOf('replace') >= 0) ||
            (chkSql.indexOf('drop') >= 0))
            return 0;
        var statement = new Statement(this, sql, errBack);
        console.log('debug1', sql);
...
var supportedEvents = [ 'trace', 'profile', 'insert' ];
